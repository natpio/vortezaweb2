...
# ==============================================================================
# 3. ZAAWANSOWANY CSS (DARK GLASSMORPHISM 2.0)
# ==============================================================================
st.markdown("""
    <style>
    ...
    .hero-main-title { font-size: 5rem; font-weight: 800; line-height: 1.05; color: #ffffff; letter-spacing: -3px; margin-bottom: 2rem; text-shadow: 0 15px 40px rgba(0,0,0,0.4); }
    ...
    .v-btn-gold:hover { transform: scale(1.05); box-shadow: 0 15px 40px rgba(189, 168, 134, 0.6); }
    ...
    .stTabs [data-baseweb="tab-list"] { background: rgba(15, 23, 42, 0.9); border-radius: 20px 20px 0 0; padding: 1rem 2rem 0 2rem; border-bottom: 2px solid #bda886; }
    .stTabs [data-baseweb="tab"] { height: 70px; color: #64748b; font-weight: 800; font-size: 1.2rem; }
    .stTabs [aria-selected="true"] { color: #bda886 !important; }
    
    .stNumberInput input, .stSelectbox select { background-color: rgba(15, 23, 42, 0.5) !important; color: white !important; border: 1px solid rgba(255,255,255,0.1) !important; }
    
    .math-box { background: rgba(255,255,255,0.03); border-radius: 12px; padding: 2rem; margin: 2rem 0; border-left: 4px solid #bda886; }
    </style>
...
def simulate_vorteza_logic(route, vehicle_type, urgency):
    ...
    time.sleep(0.5)
    logs.append(f"[{datetime.now().strftime('%H:%M:%S')}] Weryfikacja bazy Przewoźników (Local DB)...")
    
    if vehicle_type == "Bus (do 1.2t)": base_rate = 0.45
    elif vehicle_type == "Solówka (do 6t)": base_rate = 0.85
    else: base_rate = 1.15
...
st.markdown("""
    <div class="hero-container">
        <div class="hero-pretitle">Vorteza Systems</div>
        ...
        <p class="v-desc" style="max-width: 850px; margin: 0 auto 3rem auto; color: #cbd5e1;">
            Oprogramowanie B2B, które nie zgaduje problemów, lecz wynika z lat doświadczeń na pierwszej linii operacyjnej. 
            Od automatyzacji flot transportowych, po rygorystyczne przepływy danych w kancelariach i medycynie.
        </p>
...
# ==============================================================================
# 6. FILARY TECHNOLOGICZNE (THE PILLARS)
# ==============================================================================
...
# ==============================================================================
# 7. INTERAKTYWNE DEMO (VORTEZA INTELLIGENCE ENGINE)
# ==============================================================================
...
with t_log:
    ...
    with c_log1:
        ...
        <ul style="color: #cbd5e1; line-height: 1.8;">
            ...
            <li><b>Integracja Google Sheets:</b> Dane przepływają bezpośrednio z arkuszy do silnika Vorteza.</li>
            <li><b>Kontrola 360°:</b> Monitoring każdego etapu zlecenia od wyjazdu po powrót do bazy.</li>
        </ul>
        """, unsafe_allow_html=True)
    with c_log2:
        fig_map = go.Figure(data=go.Scattergeo(
            lon = [14.55, -9.13, 11.58, -93.60], lat = [53.42, 38.72, 48.13, 41.60],
            mode = 'lines+markers', line = dict(width = 2, color = '#bda886'), marker = dict(size=10, color='#ffffff')
        ))
        fig_map.update_layout(geo=dict(projection_type="orthographic", bgcolor="rgba(0,0,0,0)", showland=True, landcolor="#1e293b"), margin=dict(l=0, r=0, t=0, b=0), height=300, paper_bgcolor="rgba(0,0,0,0)")
...
with t_leg:
    ...
    with l_c1:
        st.markdown("<div style='background: rgba(255,255,255,0.05); padding: 1.5rem; border-radius: 12px;'><b style='color: #bda886;'>Repozytorium Akt</b><br>Zautomatyzowane kategoryzowanie załączników z indeksem wyszukiwania.</div>", unsafe_allow_html=True)
    with l_c2:
        st.markdown("<div style='background: rgba(255,255,255,0.05); padding: 1.5rem; border-radius: 12px;'><b style='color: #bda886;'>Generator Deadlines</b><br>Inteligentne powiadomienia o terminach ustawowych i rygorach procesowych.</div>", unsafe_allow_html=True)

with t_med:
    st.markdown("<h3 style='color: white;'>Medycyna Pracy 2.0</h3>", unsafe_allow_html=True)
    st.write("Optymalizacja czasu personelu medycznego i administracyjnego.")
    st.markdown("""
...
# ==============================================================================
# 10. VORTEZA HUB - ANALITYKA (POZIOM EKSPERT)
# ==============================================================================
st.markdown("<h2 class='v-section-header'>Analityka Systemowa</h2>", unsafe_allow_html=True)

dates = pd.date_range(end=datetime.today(), periods=30)
df = pd.DataFrame({'Data': dates, 'Manualne': np.random.randint(40, 60, size=30), 'Vorteza': np.random.randint(5, 15, size=30)})
df['Zysk_Czasu'] = df['Manualne'] - df['Vorteza']

c_an1, c_an2 = st.columns([1, 1])
with c_an1:
    fig_an1 = px.line(df, x='Data', y='Zysk_Czasu', title="Dzienny Zysk Czasu (Minuty/Operację)", color_discrete_sequence=['#bda886'])
...
with c_an2:
    fig_an2 = px.area(df, x='Data', y=['Manualne', 'Vorteza'], title="Redukcja Obciążenia Operacyjnego", color_discrete_map={'Manualne': '#475569', 'Vorteza': '#bda886'})
    ...
    <div class="v-glass-panel" style="text-align: center; padding: 5rem 2rem; border-radius: 40px; border: 1px solid #bda886;">
        ...
        <div style="margin-top: 5rem; display: flex; justify-content: space-between; align-items: center; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 2rem;">
            <div style="text-align: left;">
                ...
                <span style="color: #64748b;">Architektura & Automatyzacja B2B</span>
            </div>
            <div style="text-align: right; color: #64748b; font-size: 0.9rem;">
                <b>System:</b> Python • Streamlit • Cloud
            </div>
        </div>
    </div>
    <br><br>
